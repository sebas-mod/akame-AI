...
let handler = async (m, { conn, usedPrefix: _p, __dirname, args, command }) => {
  ...
  try {
    ...
    let wit = moment.tz('America/Buenos_Aires').format('HH:mm:ss')
    let wita = moment.tz('America/Buenos_Aires').format('HH:mm:ss')
    let wktuwib = `${wibh} H ${wibm} M ${wibs} S`

    let mode = global.opts['self'] || global.opts['owneronly'] ? 'Private' : 'Publico'
    ...
    conn.reply(m.chat, 'Lo sentimos, el menú está erróneo.', m)
    throw e
  }
}
handler.help = ['menu']
...
async function genProfile(conn, m) {
  ...
  await welcome.print(font, 550, 150, 'Nombre:');
  await welcome.print(font, 800, 150, m.pushName.slice(0, 25));
  await welcome.print(font, 550, 215, 'Info:');
  ...
  await welcome.print(font, 550, 400, 'Estado:');
